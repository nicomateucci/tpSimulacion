...
class Servidor(object):
    ...
    def arribo(self):
        ...
        if self.estadoServ == 0:
            ...
            Simulador.Completaron_demora0 += 1
        else:
            self.tiempo_servicio_total += (self.reloj - self.tiempo_ultimo_evento)
            Simulador.AreaQ0 += (Simulador.Num_cli_cola * (self.reloj - self.tiempo_ultimo_evento))
            Simulador.actualizar_tiempo_en_sistema(Simulador.Num_cli_cola * (self.reloj - self.tiempo_ultimo_evento))
            Simulador.Num_cli_cola += 1
            Simulador.Cola_inicial.append(self.reloj)
            print(len(Simulador.Cola_inicial))

    def partida(self):

        Simulador.Cola_intermedia.append(self.lista_eventos[1])
        if Simulador.Num_cli_cola > 0:

            self.lista_eventos[1] = self.reloj + generarExp(self.tiempo_entre_servicios)
            Simulador.Demora_acumulada += self.reloj - Simulador.Cola_inicial[0]
            Simulador.Completaron_demora0 += 1
            self.tiempo_servicio_total += (self.reloj - self.tiempo_ultimo_evento)
            Simulador.actualizar_tiempo_en_sistema(self.reloj - self.tiempo_ultimo_evento)
            Simulador.AreaQ0 += (Simulador.Num_cli_cola * (self.reloj - self.tiempo_ultimo_evento))
            Simulador.Num_cli_cola -= 1
            Simulador.Cola_inicial.pop(0)
        else:
            self.estadoServ = 0
    ...
    def medidasDesempeño(self):
        ...
        if self._id == 1 or self._id == 2:
            var1 = Simulador.AreaQ0 / Simulador.Reloj
            # print(Simulador.AreaQ0)
            # print(Simulador.Reloj)
            print("    Nro promedio de cli en cola: %.2f" % var1)
            var3 = Simulador.Demora_acumulada / Simulador.Completaron_demora
...
class Simulador(object):
    ...
    AreaQ0 = 0.0
    Reloj = 0.0
    Demora_acumulada0 = 0.0
    Completaron_demora0 = 0
